...
module IB
  class Order < IB::Model
    ...
    prop :tif, #  String: Time in Force (time to market): DAY/GAT/GTD/GTC/IOC
         ...
         %i[side action] => PROPS[:side], # String: Action/side: BUY/SELL/SSHORT/SSHORTX
    ...
    [:commission, # double: Shows the commission amount on the order.
     ...
     :active?,
     :inactive?,
     :complete_fill?].each { |property| define_method(property) { order_state.send(property) } }
    ...
    def place the_contract=nil, connection=nil
      ...
      modify the_contract, connection, placed_at
    end
