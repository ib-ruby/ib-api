...
module IB
  class Contract < IB::Model
    ...
    def serialize *fields # :nodoc:
      ...
      [(con_id.present? && !con_id.is_a?(Symbol) && con_id.to_i > 0 ? con_id : ''),
       ...
       (fields.include?(:sec_id_type) ? [print_default[sec_id_type], print_default[sec_id]] : nil)].flatten.compact
    end
    ...
    def serialize_under_comp *_args # :nodoc:
    ...
    def essential
      self_attributes = %i[right sec_type]
      the_attributes = %i[symbol con_id exchange
      ...
      self.class.new   the_hash.merge(self_attributes.map { |x| y = send(x); [x, y] unless y == :none}.compact.to_h)
    end

    # Contract comparison

    def == other # :nodoc:
      return false unless other.is_a?(Contract)
  ...
  class Contract
    ...
    def self.from_ib_ruby
      keys = %i[con_id symbol sec_type expiry strike right multiplier
                exchange primary_exchange currency local_symbol]
      props = Hash[keys.zip(string.split(':'))]
      props.delete_if { |_k, v| v.nil? || v.empty? }
