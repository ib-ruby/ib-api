...
module IB
  ...
  class PriceCondition < OrderCondition
    ...
    def self.make buffer
      m = new conjunction_connection: buffer.read_string,
  ...
  class TimeCondition < OrderCondition
    ...
    def self.make buffer
      new conjunction_connection: buffer.read_string,
          operator: buffer.read_int,
    ...
    def serialize
      t = self[:time]
      if t.is_a?(String) && t =~ /^\d{8}\z/
        self.time = DateTime.new t[0..3], t[4..5], t[-2..-1]
      end # expiry-format yyymmmdd
  ...
  class ExecutionCondition < OrderCondition
    ...
    def self.fabricate contract
      new contract: verify_contract_if_necessary(contract)
    end
  end
