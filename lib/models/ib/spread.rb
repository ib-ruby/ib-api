module IB
  class Spread < Bag
    ...
    def transform_distance front, back
      ...
      #	                      20180989 ---> 20.000.000
      start_date = front.to_i < 20_000_000 ? Date.strptime(front.to_s, '%Y%m') : Date.strptime(front.to_s, '%Y%m%d')
      nb = if back.to_i > 200_000
             back.to_i
           elsif back[-1] == 'w' && front.to_i > 20_000_000
             start_date + (back.to_i * 7)
           elsif back[-1] == 'm' && front.to_i > 200_000
    ...
    def fake_portfolio_position(array_of_portfolio_values)
      calculate_spread_value = lambda do |a_o_p_v, attribute|
    ...
    def add_leg contract, **leg_params
      ...
      nr =	contract.verify do |c|
        combo_legs << ComboLeg.new(c.attributes
                                        .slice(:con_id, :exchange)
                                        .merge(action: :buy)
                                        .merge(leg_params))
        self.description = begin
          description + " added #{c.to_human}"
