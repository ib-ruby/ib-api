    ...
    module Outgoing
      ...
      class AbstractMessage < IB::Messages::AbstractMessage
        ...
        def send_to socket
          ...
          # puts "------sendto ---------"
          socket.send_messages preprocess # .each {|data| socket.write_data data}
        end
        ...
        def encode
          ...
          [
            ...
            self.class.data_map.map do |(field, default_method, args)| # but request_id does
              if default_method.nil?
                @data[field]

              elsif default_method.is_a?(Symbol) # method name with args
                @data[field].send default_method, *args

              elsif default_method.respond_to?(:call) # callable with args
