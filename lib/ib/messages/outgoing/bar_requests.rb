    ...
    module Outgoing
      ...
      class BarRequestMessage < AbstractMessage
        ...
        def parse data
          ...
          error ":data_type must be one of #{DATA_TYPES.inspect}", :args unless DATA_TYPES.keys.include?(data_type)
          ...
          contract = if data[:contract].is_a?(IB::Contract)
                       data[:contract]
                     else
                       IB::Contract.from_ib_ruby(data[:contract])
                     end
      ...
      class RequestHistoricalData
        def parse data
          ...
          size = data[:bar_size] || data[:size]
          bar_size = BAR_SIZES.invert[size] || size
          error ":bar_size must be one of #{BAR_SIZES.inspect}", :args unless BAR_SIZES.keys.include?(bar_size)
          [data_type, bar_size, contract]
        end
