...
module IB
  module Messages
    ...
    module Incoming
      ...
      NewsBulletins =
        def_message 14, %i[request_id int], # unique incrementing bulletin ID.
                    %i[type int], # Type of bulletin. Valid values include:
                    #     1 = Regular news bulletin
                    #     2 = Exchange no longer available for trading
                    #     3 = Exchange is available for trading
                    %i[text string], # The bulletin's message text.
                    %i[exchange string] # Exchange from which this message originated.
      ...
      ScannerParameters = def_message 19, %i[xml xml]
      ...
      CurrentTime = def_message 49, %i[time int] # long!

      HeadTimeStamp = def_message([88, 0], %i[request_id int], %i[date int_date]) do
        # def to_human
        "<#{message_type}:" +
          "Request #{request_id}, First Historical Datapoint @ #{date}«"
      end
      ...
      FundamentalData = def_message 51, %i[request_id int], %i[xml xml]

      ContractDataEnd = def_message 52, %i[request_id int]

      OpenOrderEnd = def_message 53

      AccountDownloadEnd = def_message 54, %i[account_name string]

      ExecutionDataEnd = def_message 55, %i[request_id int]
      ...
      CommissionReport =
        def_message 59, %i[exec_id string],
                    %i[commission decimal], # Commission amount.
                    %i[currency string], # Commission currency
                    %i[realized_pnl decimal_max],
                    %i[yield decimal_max],
                    %i[yield_redemption_date int] # YYYYMMDD format

      SecurityDefinitionOptionParameter = OptionChainDefinition = def_message [75, 0],
                                                                              %i[request_id int],
                                                                              %i[exchange string],
                                                                              %i[con_id int], # underlying_con_id
                                                                              %i[trading_class string],
                                                                              %i[multiplier int]
      ...
      TickRequestParameters = def_message [81, 0], %i[ticker_id int],
                                          %i[min_tick decimal],
                                          %i[exchange string],
                                          %i[snapshot_prermissions int]
