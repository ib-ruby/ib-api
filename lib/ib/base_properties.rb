...
module IB
  ...
  module BaseProperties
    ...
    def update_missing(attrs)
      attrs = attrs.content_attributes unless attrs.is_a?(Hash)
    ...
    def ==(other)
      ...
      when String # Probably a Rails URI, delegate to AR::Base
        ...
        content_attributes.keys.inject(true) do |res, key|
    ...
    included do
      ...
      def self.define_property_methods(name, body = {})
        ...
        when Hash # recursion base case
          # Define getter
          getter = if body[:get].respond_to?(:call)
                     body[:get]
                   elsif body[:get]
                     proc { self[name].send "to_#{body[:get]}" }
                   elsif VALUES[name] # property is encoded
          ...
          # Define setter
          setter = if body[:set].respond_to?(:call)
                     body[:set]
                   elsif body[:set]
                     proc { |value| self[name] = value.send "to_#{body[:set]}" }
                   elsif CODES[name] # property is encoded
