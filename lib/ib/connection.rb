...
module IB
  ...
  class Connection
    ...
    attr_accessor :socket, :next_local_id, :client_id, :server_version, :client_version #   Socket to IB server (TWS or Gateway) # Next valid order id
    ...
    def initialize(host: '127.0.0.1',
      ...
      if connect
        ...
        Kernel.exit if next_local_id.nil?
      end
    ...
    def update_next_order_id
      i = 0
      finish = false
      sub = subscribe(:NextValidID) { finish = true }
      connected? ? send_message(:RequestIds) : open
    ...
    def connect
      ...
      socket.decode_message(socket.recieve_messages) do |the_message|
        ...
        if @server_version != MAX_CLIENT_VER
          error "ServerVersion does not match  #{@server_version} <--> #{MAX_CLIENT_VER}"
        end
      ...
      @connected = true
      logger.info do
        "Connected to server, version: #{@server_version},\n connection time: " +
    ...
    def subscribe(*args, &block)
      @subscribe_lock.synchronize do
        ...
        args.each do |what|
          message_classes =
            if what.is_a?(Class) && what < Messages::Incoming::AbstractMessage
              [what]
    ...
    def process_messages(poll_time = 50) # in msec
      ...
      while (time_left = time_out - Time.now) > 0
        ...
        # disabled for now
        next unless select [socket], nil, nil, time_left
    ...
    def send_message(what, *args)
      message =
        if what.is_a?(Messages::Outgoing::AbstractMessage)
          what
        elsif what.is_a?(Class) && what < Messages::Outgoing::AbstractMessage
          what.new(*args)
        elsif what.is_a?(Symbol)
    ...
    def subscribers
      @subscribers ||= Hash.new { |hash, subs| hash[subs] = {} }
    end
