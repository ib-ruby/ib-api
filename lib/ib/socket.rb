  ...
  refine Array do
    def tws
      if blank?
        ...
        flatten.map(&:tws).join  # [ "", [] , nil].flatten -> ["", nil]
...
module IB
  ...
  module PrepareData
    ...
    def decode_message msg
      ...
      until msg.blank?
        ...
        size = msg[0..4].unpack1('N')
        msg = msg[4..-1]
        # followed by a sequence of characters
        message = msg.unpack1("A#{size}").split("\0")
        if block_given?
          yield message
        else
  ...
  class IBSocket < TCPSocket
    ...
    def initialising_handshake
      ...
      v100_version = prepare_message Messages::SERVER_VERSION
    ...
    def send_messages *data
      syswrite prepare_message(data)
    rescue Errno::ECONNRESET => e
      Connection.logger.error do
        "Data not accepted by IB \n
		    #{data.inspect} \n
		    Backtrace:\n "
      end
