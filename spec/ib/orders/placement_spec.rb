...
describe 'Order placement'  do # :connected => true, :integration => true do
  ...
  before(:all) do
    ...
    ib.received = true
    ib.activate_plugin :symbols, :order_flow, :order_prototypes
  end
  ...
  context 'Placing wrong order', :slow => true, focus: true  do
    ...
    before(:all) do
      ...
      puts ib.received.keys
      puts ib.received[:Alert].map &:to_human
      puts ib.received[:OpenOrder]
    end
  ...
  context 'order with conditions'  do
    before(:all) do
      ib = IB::Connection.current
