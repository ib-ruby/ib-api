...
RSpec.describe IB::Messages::Incoming do
  ...
  let(:int_instruction) { IB::Messages::Incoming.def_message 10, %i[the_integer int] }
  let(:string_instruction) { IB::Messages::Incoming.def_message 10, %i[the_string string] }
  let(:decimal_instruction) {   IB::Messages::Incoming.def_message 10, %i[the_decimal decimal] }
  let(:boolean_instruction) {   IB::Messages::Incoming.def_message 10, %i[the_bool boolean] }
  let(:array_instruction) { IB::Messages::Incoming.def_message 10, %i[the_array array] }
  let(:hash_instruction) { IB::Messages::Incoming.def_message 10, %i[the_hash hash] }
  ...
  context 'Instruction with String' do
    context 'correct Behavior' do
      subject { string_instruction.new %w[1 zu] }
      it_behaves_like 'simple_instruction'
      its(:the_string) { is_expected.to be_a(String).and eq('zu') }
    end
    context 'false Integer' do
      subject { string_instruction.new %w[1 45] }
      it_behaves_like 'simple_instruction'
  ...
  context 'Instruction with Decimal' do
    ...
    context 'false Integer' do
      subject { decimal_instruction.new %w[1 45] }
      it_behaves_like 'simple_instruction'
  ...
  context 'Instruction with Array' do
    context 'correct empty Behavior' do
      subject { array_instruction.new %w[1 0] }
      it_behaves_like 'simple_instruction'
      its(:the_array) { is_expected.to be_empty }
    end
    context 'correct Behavior' do
      subject { array_instruction.new %w[1 2 eins 2] }
      it_behaves_like 'simple_instruction'
      its(:the_array) { is_expected.to eq %w[eins 2] }
    end
    context 'false  String' do
      subject { array_instruction.new %w[1 Zted] } # perhaps request nil as answer?
      it_behaves_like 'simple_instruction'
      its(:the_array) { is_expected.to be_empty }
    end
    context 'false  Integer' do
      subject { array_instruction.new %w[1 45] }
      it_behaves_like 'simple_instruction'
  ...
  context 'Instruction with Hash' do
    context 'correct empty Behavior' do
      subject { hash_instruction.new %w[1 0] }
      it_behaves_like 'simple_instruction'
      its(:the_hash) { is_expected.to be_empty }
    end
    context 'correct Behavior' do
      subject { hash_instruction.new %w[1 2 eins 2 fuenf zurHeide] }
      it_behaves_like 'simple_instruction'
      its(:the_hash) { is_expected.to eq eins: '2', fuenf: 'zurHeide' }
    end
    context 'false  String' do
      subject { hash_instruction.new %w[1 Zted] } # perhaps request nil as answer?
      it_behaves_like 'simple_instruction'					 # now: because of "abc".to_i = 0 an empty hash is  created
      its(:the_hash) { is_expected.to be_empty }
    end
    context 'false  Integer' do
      subject { hash_instruction.new %w[1 45] }
      it_behaves_like 'simple_instruction'
