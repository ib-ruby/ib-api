...
RSpec.describe "IB::Strangle" do
  let ( :the_option ){ IB::Symbols::Options.stoxx.merge strike: PUT_ESTX }
  before(:all) do
  ...
  context "build with underlying" do
    subject{ IB::Strangle.build from: IB::Symbols::Index.stoxx, p: PUT_ESTX, c: CALL_ESTX }

    it{ is_expected.to be_a IB::Spread }
    it_behaves_like 'a valid Estx Combo'
  end
  context "build with option"  do
    subject{ IB::Strangle.build from: the_option, c: CALL_ESTX }
